...
<?php if ($mode !== 'view'): ?>
  ...
  <div class="card border-0 shadow-sm mb-4">
    <div class="card-body">
      <h5 class="fw-semibold mb-3" style="color:#134557;">
        <?= $mode === 'update' ? 'Upload Updated Excel File' : 'Upload Excel File' ?>
      </h5>

      <p class="text-muted small mb-3">
        Excel file must have these columns in order:
        <strong>Code, Test Name, Rate, Sample, Reporting Time</strong>
      </p>

      <form action="<?= base_url('labs/' . $lab['id'] . '/pricelist' . ($mode === 'update' ? '/update' : '')) ?>"
